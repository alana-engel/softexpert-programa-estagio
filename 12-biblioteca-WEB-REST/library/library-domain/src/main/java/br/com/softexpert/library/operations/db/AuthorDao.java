...
public class AuthorDao implements Operations<Author>{
	...
	static final String SQL_SEARCH = "select * from tbl_author where sequentialCode=?";
	static final String SQL_SEARCH_BY_NAME = "select * from tbl_author where name=?";
	public AuthorDao() {
	...
	public Author searchByName(String name) throws Exception {
		...
		try (Connection connection = new ConnectionFactory().getConnection();
				PreparedStatement stmt = connection.
				prepareStatement(SQL_SEARCH_BY_NAME);){
