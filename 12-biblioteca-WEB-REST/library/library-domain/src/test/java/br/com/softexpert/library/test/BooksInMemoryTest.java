...
public class BooksInMemoryTest {
	...
	public void testDeleteBook() throws Exception{
		...
		book.setAuthorsList(createAuthor());
		book.setCategory(createCategory());
		book.setSequentialCode(20);
		assertTrue(booksInMemory.create(book));
		List<Book> listBook = new ArrayList<Book>();
		listBook.add(book);
		assertEquals(booksInMemory.searchByTitle("livroExcluir"),listBook);
		assertTrue(booksInMemory.delete(listBook.get(0).getSequentialCode()));
		booksInMemory.searchByTitle("livroExcluir");
	}

	@Test
	public void testDeleteBookNotExists(){
		assertFalse(booksInMemory.delete(-20));
	}
